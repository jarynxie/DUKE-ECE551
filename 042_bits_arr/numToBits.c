...
void numToBits(uint32_t * nums, int nNums, int * bits, int nBits) {
  ...
  for (int numsPos = 0; numsPos < nNums; numsPos++) {
    for (int numsLoc = 0, bitsPos = 0; numsLoc <= 31; numsLoc++, bitsPos++) {
      bits[numsPos * 32 + bitsPos] = getNthBit(nums[numsPos], 31 - numsLoc);
    }
  }
}
